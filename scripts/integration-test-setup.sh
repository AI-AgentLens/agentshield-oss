...
# --- Codex setup ---
# Codex CLI's PreToolUse hook uses the same JSON payload shape as Claude Code
# plus a Codex-only `turn_id` (used for audit-log source labeling). These
# tests mirror the Claude Code section so regressions in either path are
# caught — they are also the regression gate for #1738 (the SessionStart
# placeholder we used to write before Codex shipped PreToolUse).
echo "=== [6/6] Codex setup ==="

# 6a: Setup into empty ~/.codex (fresh install)
rm -rf "$HOME/.codex"
mkdir -p "$HOME/.codex"
agentshield setup codex > /dev/null 2>&1 || true
if [ -f "$HOME/.codex/hooks.json" ] && grep -q "agentshield hook" "$HOME/.codex/hooks.json"; then
    pass "codex: hook installed"
else
    fail "codex: hook not found in hooks.json"
fi

# 6b: Bash matcher is anchored — guards against future `BashOutput` tools
if grep -q '"\^Bash\$"' "$HOME/.codex/hooks.json"; then
    pass "codex: matcher anchored to ^Bash$"
else
    fail "codex: matcher not anchored (would over-match)"
fi

# 6c: Setup with pre-existing other hooks (must not destroy them)
cat > "$HOME/.codex/hooks.json" <<HOOKS
{
  "hooks": {
    "SessionEnd": [
      {
...
if grep -q "SessionEnd" "$HOME/.codex/hooks.json" && \
   grep -q "echo bye" "$HOME/.codex/hooks.json" && \
   grep -q "agentshield hook" "$HOME/.codex/hooks.json"; then
    pass "codex: merged without destroying existing hooks"
else
    fail "codex: merge destroyed existing hooks"
fi

# 6d: Legacy SessionStart placeholder is swept on upgrade
# Pre-v0.2 versions of `agentshield setup codex` wrote a SessionStart echo
# hook because Codex didn't yet support PreToolUse. We migrate users off it.
cat > "$HOME/.codex/hooks.json" <<LEGACY
{
  "hooks": {
    "SessionStart": [
      {
...
LEGACY
agentshield setup codex > /dev/null 2>&1
if grep -q "Session guard active" "$HOME/.codex/hooks.json"; then
    fail "codex: legacy SessionStart placeholder not swept"
elif grep -q "agentshield hook" "$HOME/.codex/hooks.json"; then
    pass "codex: legacy SessionStart placeholder swept on upgrade"
else
    fail "codex: upgrade left hooks.json in unexpected state"
fi

# 6e: Idempotent re-run (no duplicate entries)
agentshield setup codex > /dev/null 2>&1
COUNT=$(grep -c "agentshield hook" "$HOME/.codex/hooks.json")
if [ "$COUNT" -eq 1 ]; then
    pass "codex: idempotent (no duplicate)"
else
    fail "codex: duplicate entries ($COUNT found)"
fi

# 6f: Hook payload routing — safe Bash command (Codex payload shape with turn_id)
EXIT=$(hook_eval '{"hook_event_name":"PreToolUse","tool_name":"Bash","turn_id":"turn_e2e_safe","tool_input":{"command":"echo hello"}}')
if [ "$EXIT" = "0" ]; then
    pass "codex: hook allows safe Bash command (exit 0)"
else
    fail "codex: hook returned exit $EXIT for safe command"
fi

# 6g: Hook payload routing — dangerous command must BLOCK (exit 2)
EXIT=$(hook_eval '{"hook_event_name":"PreToolUse","tool_name":"Bash","turn_id":"turn_e2e_block","tool_input":{"command":"rm -rf /"}}')
if [ "$EXIT" = "2" ]; then
    pass "codex: hook blocks dangerous command (exit 2)"
else
    fail "codex: hook returned exit $EXIT for rm -rf / (expected 2)"
fi

# 6h: Codex payload labels audit source as codex-hook (turn_id present)
if grep -q '"source":"codex-hook"' "$HOME/.agentshield/audit.jsonl" 2>/dev/null; then
    pass "codex: audit log labels source as codex-hook"
else
    fail "codex: audit log missing codex-hook source label"
fi

# 6i: Same payload WITHOUT turn_id labels as claude-code-hook (no Codex signal)
hook_eval '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"echo no-turn-id"}}' > /dev/null
if tail -1 "$HOME/.agentshield/audit.jsonl" 2>/dev/null | grep -q '"source":"claude-code-hook"'; then
    pass "codex: payload without turn_id falls back to claude-code-hook label"
else
    fail "codex: source labeling regressed (turn_id-less payload not labelled claude-code-hook)"
fi

# 6j: Disable with surgical removal (preserves other hooks added in 6c flow)
cat > "$HOME/.codex/hooks.json" <<HOOKS
{
  "hooks": {
    "SessionEnd": [
    ...
    "PreToolUse": [
      {
        "matcher": "^Bash\$",
        "hooks": [{"type": "command", "command": "/usr/local/bin/agentshield hook", "timeout": 10}]
      }
    ]
  }
}
...
if grep -q "agentshield hook" "$HOME/.codex/hooks.json" 2>/dev/null; then
    fail "codex: disable did not remove hook"
elif grep -q "SessionEnd" "$HOME/.codex/hooks.json" && grep -q "echo bye" "$HOME/.codex/hooks.json"; then
    pass "codex: disable removed hook surgically, preserved other entries"
else
    fail "codex: disable damaged other entries"
fi

# 6k: Disable when AgentShield was the only entry → file backed up, not edited
rm -rf "$HOME/.codex"
mkdir -p "$HOME/.codex"
agentshield setup codex > /dev/null 2>&1
agentshield setup codex --disable > /dev/null 2>&1
if [ ! -f "$HOME/.codex/hooks.json" ] && [ -f "$HOME/.codex/hooks.json.bak" ]; then
    pass "codex: disable backs up file when no other entries remain"
else
    fail "codex: disable did not back up empty-after-removal file"
fi
echo ""
