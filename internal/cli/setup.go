...
`
	...
	fmt.Println()
	fmt.Println("For full command interception, Codex needs a native PreToolUse hook.")
	fmt.Println("Track progress on first-class Codex support in issue tracker.")
	fmt.Println()
	fmt.Println("To disable: agentshield setup codex --disable")
	fmt.Println()
	printStatus()
