...
// TestLoad_EnforcementMode_RoundTrip — verifies that the SaaS-pushed YAML's
// top-level `enforcement_mode:` field deserializes onto Policy.EnforcementMode.
// The contract is owned cross-repo: aiagentlens emits this field via
// /api/policy/yaml (rulepush.go:138). Before this test it was silently
// dropped because the Policy struct didn't have the field — the SaaS
// Settings toggle in app.aiagentlens.com was a functional no-op for any
// agent in the enterprise sync path. Caught by the 2026-05-17 deep-dive.
// Issue #1952.
func TestLoad_EnforcementMode_RoundTrip(t *testing.T) {
	path := filepath.Join(t.TempDir(), "policy.yaml")
	must(t, os.WriteFile(path, []byte(`version: "0.1"
...
`), 0o600))
	...
	if pol.EnforcementMode != "audit-only" {
		t.Errorf("Policy.EnforcementMode = %q; want %q (SaaS-pushed field must round-trip through the YAML loader)",
			pol.EnforcementMode, "audit-only")
	}
}

// TestLoad_EnforcementMode_AbsentIsEmpty — when the SaaS-pushed YAML omits
// enforcement_mode (e.g. an older aiagentlens version pre-dating issue #1952),
// Policy.EnforcementMode is the empty string. config.Load then falls through
// to the next resolution rung. Documents the "no opinion" semantics.
func TestLoad_EnforcementMode_AbsentIsEmpty(t *testing.T) {
	path := filepath.Join(t.TempDir(), "policy.yaml")
	must(t, os.WriteFile(path, []byte(`version: "0.1"
rules: []
`), 0o600))
	...
	if pol.EnforcementMode != "" {
		t.Errorf("Policy.EnforcementMode = %q; want empty (absent field must not synthesize a value)", pol.EnforcementMode)
	}
}
