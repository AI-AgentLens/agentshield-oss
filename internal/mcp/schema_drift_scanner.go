...
type SchemaDriftResult struct {
	...
	// OutputSchemaChangedTools lists tool names whose outputSchema (MCP 2025-06-18)
	// changed since baseline while the input schema stayed stable. The outputSchema
	// describes the shape of a tool's structured result and is shown to the LLM during
	// tools/list — so a post-approval swap is an output-side rug-pull: it re-shapes
	// how the agent extracts and trusts result data, and can re-introduce poisoned
	// example/enum values, without touching the input schema the original drift check
	// fingerprints. Tracked separately because input-side and output-side mutations
	// have distinct threat models (argument poisoning vs result-interpretation poisoning).
	OutputSchemaChangedTools []string
...
type toolSchemaEntry struct {
	...
	// OutputSchemaHash is the SHA-256 of the tool's outputSchema JSON (MCP 2025-06-18).
	// A change here is an output-side rug-pull indicator. Empty when the tool declares
	// no outputSchema and on caches written before this field existed — drift on this
	// field is therefore only reported when a prior non-empty hash exists (upgrade-safe).
...
func (s *SchemaDriftScanner) CheckDrift(serverName string, tools []ToolDefinition) *SchemaDriftResult {
	...
	for name := range current.Tools {
		...
		if !existed {
			...
			// Detect output-schema drift (output-side rug-pull): the outputSchema swapped
			// post-approval. Guarded on a prior non-empty hash so caches written before this
			// field existed do not produce a spurious drift on first upgrade. New caches store
			// "empty" (not "") for tools with no outputSchema, so a tool that GAINS an
			// outputSchema after upgrade is still correctly flagged.
			if cachedEntry.OutputSchemaHash != "" &&
				current.Tools[name].OutputSchemaHash != cachedEntry.OutputSchemaHash {
				result.OutputSchemaChangedTools = append(result.OutputSchemaChangedTools, name)
				result.Drifted = true
			}
		}
	}
	...
	sort.Strings(result.OutputSchemaChangedTools)
...
func buildSchemaCache(tools []ToolDefinition) schemaCache {
	...
	for _, t := range tools {
		cache.Tools[t.Name] = toolSchemaEntry{
			...
			OutputSchemaHash: hashInputSchema(t.OutputSchema),
		}
	}
	return cache
}
...
func (r *SchemaDriftResult) DriftSummary() string {
	...
	if len(r.OutputSchemaChangedTools) > 0 {
		parts = append(parts, fmt.Sprintf("output-schema rug-pull: [%s]", strings.Join(r.OutputSchemaChangedTools, ", ")))
	}
