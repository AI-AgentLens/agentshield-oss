...
// TestLoadMCPPacksExcluding_SkipsByPackName — issue #1628. The runtime loader
// uses this to prevent ~/.agentshield/packs/mcp (legacy fallback) from
// re-merging stale copies of packs already provided by the embedded layer.
func TestLoadMCPPacksExcluding_SkipsByPackName(t *testing.T) {
	dir := t.TempDir()
	keepPack := `name: keep-me
version: "1.0"
blocked_tools: ["only_in_keep"]
`
	stalePack := `name: stale-duplicate
version: "0.9"
blocked_tools: ["only_in_stale"]
`
	if err := os.WriteFile(filepath.Join(dir, "keep.yaml"), []byte(keepPack), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "stale.yaml"), []byte(stalePack), 0o644); err != nil {
		t.Fatal(err)
	}

	base := &MCPPolicy{Defaults: MCPDefaults{Decision: policy.DecisionAudit}}

	// Caller signals "stale-duplicate" was already loaded from a higher-priority
	// source (e.g., embedded). The disk version must be skipped entirely.
	exclude := map[string]bool{"stale-duplicate": true}
	result, infos, err := LoadMCPPacksExcluding(dir, base, exclude)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if len(infos) != 1 || infos[0].Name != "keep-me" {
		t.Errorf("infos = %+v, want exactly [keep-me]", infos)
	}

	hasKeep := false
	hasStale := false
	for _, bt := range result.BlockedTools {
		if bt == "only_in_keep" {
			hasKeep = true
		}
		if bt == "only_in_stale" {
	...
	if !hasKeep {
		t.Error("BlockedTools missing 'only_in_keep' — non-excluded pack should still load")
	}
	if hasStale {
		t.Error("BlockedTools contains 'only_in_stale' — excluded pack should NOT have been merged")
	}
}

// TestLoadMCPPacksExcluding_NilExcludeBehavesLikeLoadMCPPacks — guards against
// regressing the public LoadMCPPacks contract when excludeNames is nil.
func TestLoadMCPPacksExcluding_NilExcludeBehavesLikeLoadMCPPacks(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "p.yaml"), []byte(
		"name: p\nversion: \"1.0\"\nblocked_tools: [\"t\"]\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	base := &MCPPolicy{Defaults: MCPDefaults{Decision: policy.DecisionAudit}}

	wrapped, _, err := LoadMCPPacks(dir, base)
	if err != nil {
		t.Fatalf("LoadMCPPacks: %v", err)
	}
	excluded, _, err := LoadMCPPacksExcluding(dir, base, nil)
	if err != nil {
		t.Fatalf("LoadMCPPacksExcluding: %v", err)
	}

	if len(wrapped.BlockedTools) != len(excluded.BlockedTools) {
		t.Errorf("nil-exclude diverges from LoadMCPPacks: wrapped=%v excluded=%v",
			wrapped.BlockedTools, excluded.BlockedTools)
	}
}
...
func TestLoadMCPPacks_InvalidYAML(t *testing.T) {
	...
	// Issue #2188: the parse failure must be recorded, not silently swallowed.
	if infos[0].LoadError == nil {
		t.Error("invalid MCP pack must record LoadError, got nil — #2188 regression")
	}
	if infos[0].RuleCount != 0 {
		t.Errorf("invalid MCP pack must report 0 rules, got %d", infos[0].RuleCount)
	}
	failed := FailedMCPPacks(infos)
	if len(failed) != 1 {
		t.Fatalf("expected FailedMCPPacks to return 1 pack, got %d", len(failed))
	}
