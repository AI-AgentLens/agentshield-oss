...
func TestParseMessage_OtherRequest(t *testing.T) {
	input := `{"jsonrpc":"2.0","id":6,"method":"logging/setLevel","params":{"level":"debug"}}`
...
// TestParseMessage_CompletionComplete — issue #2791, follow-up to #2789:
// completion/complete now has its own MessageKind so its request-side
// `argument.value` field can be scanned for outbound content leakage,
// matching the prompts/get `arguments` map handling.
func TestParseMessage_CompletionComplete(t *testing.T) {
	input := `{"jsonrpc":"2.0","id":6,"method":"completion/complete","params":{"ref":{"type":"ref/prompt","name":"test"},"argument":{"name":"lang","value":"py"}}}`
	...
	if kind != KindCompletionComplete {
		t.Errorf("expected KindCompletionComplete, got %v", kind)
	}
}
