...
func ExtractGetPromptParams(msg *Message) (*GetPromptParams, error) {
	if msg.Method != MethodPromptsGet {
		return nil, fmt.Errorf("not a prompts/get request: method=%q", msg.Method)
	}
	if msg.Params == nil {
		return nil, fmt.Errorf("prompts/get request has no params")
	}
	...
	if params.Name == "" {
		return nil, fmt.Errorf("prompts/get params missing required field 'name'")
	}
	return &params, nil
}
...
func ExtractCompletionCompleteParams(msg *Message) (*CompletionCompleteParams, error) {
	if msg.Method != MethodCompletionComplete {
		return nil, fmt.Errorf("not a completion/complete request: method=%q", msg.Method)
	}
	if msg.Params == nil {
		return nil, fmt.Errorf("completion/complete request has no params")
	}

	var params CompletionCompleteParams
	if err := json.Unmarshal(msg.Params, &params); err != nil {
		return nil, fmt.Errorf("failed to parse completion/complete params: %w", err)
	}
	if params.Argument.Name == "" {
		return nil, fmt.Errorf("completion/complete params missing required field 'argument.name'")
	}
	return &params, nil
}
