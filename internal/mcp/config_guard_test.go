...
func TestConfigGuard_NpmrcNotBlockedByGuard(t *testing.T) {
	// .npmrc is intentionally NOT blocked by the config guard (issue #1897).
	// Enterprise registries (Verdaccio, Artifactory, etc.) require writing .npmrc.
	// The YAML rule mcp-sc-block-npmrc-write handles this at AUDIT severity instead.
	home := os.Getenv("HOME")
	result := CheckConfigGuard("write_file", map[string]interface{}{
		"path":    home + "/.npmrc",
	...
	if result.Blocked {
		t.Errorf("expected NOT blocked by config guard — .npmrc handled by YAML AUDIT rule (issue #1897), got: %v", result.Findings)
	}
}
...
func TestConfigGuard_PipConfNotBlockedByGuard(t *testing.T) {
	// pip.conf is intentionally NOT blocked by the config guard (issue #1897 follow-up).
	// Enterprise PyPI mirrors require writing pip.conf. The generated community rule
	// mcp-gen-protected-path-pip (**/.pip/**) and mcp-gen-protected-path-config-pip
	// (**/.config/pip/**) still block it via YAML policy.
	home := os.Getenv("HOME")
	result := CheckConfigGuard("write_file", map[string]interface{}{
	...
	if result.Blocked {
		t.Errorf("expected NOT blocked by config guard — pip.conf handled by YAML rules, got: %v", result.Findings)
	}
}
