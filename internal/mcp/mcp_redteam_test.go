...
func TestMCPRedTeam_CombinedSession(t *testing.T) {
	...
	msgs := []string{
		...
		// 6. AUDIT by YAML rule — write .npmrc (config guard removed, issue #1897)
	...
	assertResponseAllowed(t, responses, 6) // AUDIT — enterprise .npmrc write (issue #1897)
	...
	// 4 blocked tool calls + 1 poisoned description = 5 BLOCK entries
	if blocked != 5 {
		t.Errorf("expected 5 BLOCK entries, got %d", blocked)
		for _, e := range audit {
			t.Logf("  %s: %s (rules: %v)", e.ToolName, e.Decision, e.TriggeredRules)
		}
	}
	// Baseline: 4 AUDIT tool calls (get_weather, read_file, send_tokens small,
	// .npmrc write) — each pinned by the per-call assertions above. Defense-in-depth
	// scanners add further AUDIT *audit-log* entries on top without changing any
	// per-call decision: the deliberately-poisoned tools/list trips
	// mcp-tool-name-collision, and response-content scanners (#2555/#2557) may flag
	// reflected responses. The exact extra count is Go-scheduler/-version dependent
	// (4 on CI's Go 1.23, more under parallel load), so assert a lower bound here and
	// rely on the per-message assertions above to pin the actual security behavior.
	if audited < 4 {
		t.Errorf("expected at least 4 AUDIT entries, got %d", audited)
		for _, e := range audit {
			t.Logf("  %s: %s (rules: %v)", e.ToolName, e.Decision, e.TriggeredRules)
		}
	}
