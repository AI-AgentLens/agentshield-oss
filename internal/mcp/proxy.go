...
func (p *Proxy) Run() error {
	...
	// Both proxy directions write to the client (stdout): the client→server
	// goroutine writes JSON-RPC error responses for blocked calls, while the
	// server→client goroutine writes the passthrough/scanned stream. Guard
	// stdout with a mutex so those writes cannot interleave and corrupt a
	// JSON-RPC frame. RunWithIO already wraps its client writer this way; Run()
	// previously passed raw os.Stdout to both goroutines (a data race).
	clientOut := &lockedWriter{w: os.Stdout}
	...
	go func() {
		...
		p.proxyClientToServer(os.Stdin, serverStdin, clientOut)
	}()
	...
	go func() {
		defer wg.Done()
		p.proxyServerToClient(serverStdout, clientOut)
	}()
...
func (p *Proxy) proxyClientToServer(clientReader io.Reader, serverWriter io.Writer, clientWriter io.Writer) {
	...
	for scanner.Scan() {
		...
		if kind == KindPromptsGet {
			blocked, blockResp := p.handler.HandlePromptsGetRequest(msg)
			if blocked {
				writeLineToWriter(clientWriter, blockResp)
				continue
			}
		...
		if kind == KindCompletionComplete {
			blocked, blockResp := p.handler.HandleCompletionCompleteRequest(msg)
			if blocked {
				writeLineToWriter(clientWriter, blockResp)
				continue
			}
		...
		if kind == KindTasksGet || kind == KindTasksResult {
			p.handler.HandleTaskPollRequest(msg)
		}
		...
		if kind == KindResponse {
			if replacement := p.handler.HandleRootsListResponse(line); replacement != nil {
				writeLineToWriter(clientWriter, replacement)
				continue
			}
		}
...
// proxyServerToClient reads messages from the MCP server, scans responses for
// poisoned content, and intercepts server-initiated requests/notifications,
// then forwards to the client.
//
// Performance: rather than running every response scanner on every message
// (each re-parsing the envelope), it parses the envelope once and routes
// responses to the single applicable scanner via DispatchServerResponse. See
// the safety invariant documented on that method.
func (p *Proxy) proxyServerToClient(serverReader io.Reader, clientWriter io.Writer) {
	...
	for scanner.Scan() {
		...
		// Responses (id, no method): route to the single applicable scanner.
		if msg.Method == "" {
			if filtered := p.handler.DispatchServerResponse(msg, line); filtered != nil {
		...
		case KindNotification:
			...
			// Drop notifications/resources/updated if the URI matches a blocked resource.
			// A compromised server can redirect update notifications to credential paths.
			if p.handler.HandleResourcesUpdatedNotification(msg) {
				continue
			}
			// Record pending capability expansion check when server signals tool list changed.
			p.handler.HandleToolsListChangedNotification(msg)
			// Drop notifications/progress if the message field contains injection patterns.
			// A compromised server can embed adversarial instructions in progress messages
			// that arrive without a prior tool call, bypassing argument-level scanning.
			if p.handler.HandleProgressNotification(msg) {
				continue
			}
		}
