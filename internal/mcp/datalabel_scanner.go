...
// ConvertDataLabels converts policy.DataLabel YAML types into the engine-side
// datalabel.DataLabelConfig representation used by the MCP data label scanner.
// Mirrors policy.convertDataLabels (used by the shell pipeline) — kept aligned
// so a YAML schema field cannot silently disappear on one path only.
func ConvertDataLabels(labels []policy.DataLabel) []datalabel.DataLabelConfig {
