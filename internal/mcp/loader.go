...
type MCPPack struct {
	...
	// Generated is an advisory timestamp on generator-produced packs (e.g.
	// mcp-generated.yaml, produced by cmd/mcp-gen) — metadata only.
	Generated string `yaml:"generated,omitempty"`
}

// MCPPackInfo describes a loaded MCP pack for reporting. It is an alias of
// policy.PackInfo — the shell and MCP loader families share the same listing
// shape, including the issue-#2188 LoadError semantics: when LoadError is
// non-nil, RuleCount is 0 and none of that pack's rules were loaded (MCP
// mediation for the pack is absent), but loading continues for other packs
// and the failure is recorded rather than silently swallowed.
type MCPPackInfo = policy.PackInfo

// FailedMCPPacks returns the subset of infos whose pack failed to parse
// (LoadError != nil). Testable seam mirroring policy.FailedPacks, used by the
// MCP proxies and scan diagnostic to surface degraded MCP enforcement.
func FailedMCPPacks(infos []MCPPackInfo) []MCPPackInfo {
	return policy.FailedPacks(infos)
}
...
func LoadMCPPacks(packsDir string, base *MCPPolicy) (*MCPPolicy, []MCPPackInfo, error) {
	return LoadMCPPacksExcluding(packsDir, base, nil)
}

// LoadMCPPacksExcluding behaves like LoadMCPPacks but skips any pack whose
// resolved Name appears in excludeNames. Used by the runtime loader so the
// legacy ~/.agentshield/packs/mcp fallback does not re-merge stale copies of
// packs already provided by the embedded layer (issue #1628). The check uses
// the pack's declared Name (or its filename stem if Name is empty) — same key
// used in MCPPackInfo.Name, so callers can build excludeNames directly from a
// prior LoadMCPPacks/LoadEmbeddedMCPPacks result.
func LoadMCPPacksExcluding(packsDir string, base *MCPPolicy, excludeNames map[string]bool) (*MCPPolicy, []MCPPackInfo, error) {
	sources, err := policy.DiskPackSources(packsDir, nil)
	if err != nil {
		if os.IsNotExist(err) {
			return base, nil, nil
		}
	...
	infos := policy.LoadPackSources(sources, policy.PackLoadHooks[MCPPack]{
		Parse: func(data []byte, label string) (*MCPPack, error) {
			var pack MCPPack
			if err := yaml.Unmarshal(data, &pack); err != nil {
				return nil, fmt.Errorf("failed to parse MCP pack %s: %w", label, err)
			}
			return &pack, nil
		},
		Describe: func(p *MCPPack) MCPPackInfo {
			return MCPPackInfo{
				Name:        p.Name,
				Description: p.Description,
				Version:     p.Version,
				Author:      p.Author,
				RuleCount:   mcpPackRuleCount(p),
			}
		},
		// Skip silently when the caller already has this pack from a higher
		// priority source (typically embedded, #1628). Do NOT add to infos —
		// surfacing it would imply it contributed rules.
		SkipPack: func(info MCPPackInfo) bool { return excludeNames[info.Name] },
		Merge:    func(p *MCPPack) { mergeMCPPack(result, p) },
	})
...
//
// Embedded MCP packs are always enabled and are guarded at build time by
// TestEmbeddedMCPPacks_RuleCountFloor; parse failures are still recorded as
// LoadError to keep the runtime path honest (issue #2188).
func LoadEmbeddedMCPPacks(base *MCPPolicy) (*MCPPolicy, []MCPPackInfo, error) {
	...
	infos := policy.LoadPackSources(policy.EmbeddedPackSources(embeddedFiles), policy.PackLoadHooks[MCPPack]{
		Parse: func(data []byte, label string) (*MCPPack, error) {
			var pack MCPPack
			if err := yaml.Unmarshal(data, &pack); err != nil {
				return nil, fmt.Errorf("failed to parse embedded MCP pack %s: %w", label, err)
			}
			return &pack, nil
		},
		// Embedded listing intentionally carries only Name/Version/RuleCount
		// (no Description/Author) — matching the pre-refactor output shape.
		Describe: func(p *MCPPack) MCPPackInfo {
			return MCPPackInfo{
				Name:      p.Name,
				Version:   p.Version,
