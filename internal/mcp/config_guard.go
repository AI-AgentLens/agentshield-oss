...
func init() {
	...
	templates := []protectedConfigPattern{
		...
		// NOTE (issue #1897): ~/.npmrc is intentionally NOT in this list. Enterprise customers
		// legitimately write .npmrc to configure private registries (Verdaccio, Artifactory,
		// GitHub Packages, AWS CodeArtifact). The mcp-sc-block-npmrc-write YAML rule (AUDIT)
		// handles .npmrc writes at the appropriate severity. Keeping it here would create a
		// defense-in-depth BLOCK that wrongly breaks every enterprise npm registry setup.
		// NOTE (issue #1897 follow-up): pip.conf is also intentionally excluded — enterprise
		// private PyPI mirrors require writing pip.conf. Currently still BLOCK via the
		// generated community rule mcp-gen-protected-path-pip (**/.pip/**); when that rule
		// is also downgraded, remove it from here too.
		...
		// Claude Code primary state file — contains oauthAccount OAuth tokens
		{"~/.claude.json", "ide-mcp-config", "Writing to ~/.claude.json could replace Claude Code's OAuth tokens or tamper with project configurations."},
	}
