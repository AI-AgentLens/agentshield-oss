...
func runTestCases(t *testing.T, cases []testdata.TestCase) {
	...
	for _, tc := range cases {
		t.Run(tc.ID, func(t *testing.T) {
			...
			if tc.Analyzer == "structural" || tc.Analyzer == "semantic" || tc.Analyzer == "stateful" || tc.Analyzer == "dataflow" || tc.Analyzer == "pipeline" || tc.Analyzer == "guardian" || tc.Analyzer == "substitution" || tc.Analyzer == "datalabel" {
...
func loadTestPolicy(t *testing.T) *policy.Policy {
	...
	// Append a fixture data label so the conditional Layer 7 registration in
	// BuildAnalyzerPipeline (len(pol.DataLabels) > 0) is exercised by the
	// corpus — no shipped pack carries data_labels, so without this the
	// datalabel stage never registers in any test. Asserted by
	// testdata.DataLabelFixtureCases (TP + TN). The keyword is synthetic and
	// case-sensitive so it cannot collide with any other corpus command.
	pol.DataLabels = append(pol.DataLabels, policy.DataLabel{
		ID:         "test-fixture-codename",
		Name:       "Test Fixture Codename",
		Decision:   policy.DecisionBlock,
		Confidence: 0.95,
		Reason:     "customer-defined codename AALFIXTURE-CODENAME-ZUNIQ7 must not appear in shell commands",
		Patterns: []policy.DataLabelPattern{
			{Keywords: []string{"AALFIXTURE-CODENAME-ZUNIQ7"}, CaseSensitive: true},
		},
	})
	return pol
}
...
func TestGenerateFailingTestsReport(t *testing.T) {
	...
	b.WriteString("> Auto-generated by `TestGenerateFailingTestsReport`\n")
	b.WriteString("> Run: `go test -v -run TestGenerateFailingTestsReport ./internal/analyzer/`\n\n")

	b.WriteString("## Summary\n\n")
	b.WriteString("| Metric | Count |\n")
	b.WriteString("|--------|-------|\n")
	fmt.Fprintf(&b, "| Total failing | %d |\n", len(failures))
	fmt.Fprintf(&b, "| False Negatives (missed threats) | %d |\n", fnCount)
	fmt.Fprintf(&b, "| False Positives (false alarms) | %d |\n", fpCount)
	fmt.Fprintf(&b, "| Total test cases | %d |\n", len(allCases))
