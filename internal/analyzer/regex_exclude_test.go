...
// docContextLabels is the standard doc-context suppression set shipped on
// rules as `command_intent_exclude` (the successor of the retired
// {{DOC_CONTEXT}} regex macro).
var docContextLabels = []string{LabelIsBashComment, LabelIsDocText, LabelInHeredoc, LabelIsSelfMgmt}

// matchRegexRuleWithIntent mirrors how the pipeline (regex.go Analyze) and the
// regex-fallback engine apply a rule that carries IntentExclude labels: the
// regex must match AND IntentExcludedForStatements must not suppress it.
// Delegates to the real production helper (rather than re-deriving the check
// here) so this test file can't silently drift from actual runtime behavior —
// see IntentExcludedForStatements's doc comment for why whole-command
// HasAny() alone is wrong for compound commands.
func matchRegexRuleWithIntent(c *IntentClassifier, command string, rule RegexRule) bool {
	if !matchRegexRule(command, rule) {
		return false
	}
...
func TestRegexExcludeSSHPrivate(t *testing.T) {
	...
	tests := []struct {
		...
		// python3 -c with SSH path as string literal (issue #1788)
		{"python3 -c with ssh path in string", `python3 -c "old = 'call_tool with file:///.ssh/id_rsa'; print(old)"`, false},
		{"python -c with ssh path in string", `python -c "ref = '~/.ssh/id_ed25519'; print(ref)"`, false},
		{"python3 -c after cd compound", `cd /tmp && python3 -c "s = '.ssh/id_rsa'; print(s)"`, false},
	}
...
func TestPasteServiceDeadDropFPRegression(t *testing.T) {
	...
	rule := RegexRule{
		...
		Decision:      "BLOCK",
		Regex:         mainRegex,
		IntentExclude: docContextLabels,
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := matchRegexRuleWithIntent(c, tt.command, rule)
			if got != tt.want {
				t.Errorf("matchRegexRuleWithIntent(%q) = %v, want %v", tt.command, got, tt.want)
			}
		})
	}
}
...
// TestTCCUtilCompoundHeredocFPRegression verifies that ts-block-macos-tccutil-reset
// does not block heredoc writes (cat/tee) that mention "tccutil reset" as text content,
// even when the cat/tee follows cd in a compound command (issue #1778).
func TestTCCUtilCompoundHeredocFPRegression(t *testing.T) {
	mainRegex := "(?:^|[\\s;&|()`]|\\bsudo\\s+)tccutil\\s+reset\\b"

	c := NewIntentClassifier()
	rule := RegexRule{
		ID:            "ts-block-macos-tccutil-reset",
		Decision:      "BLOCK",
		Regex:         mainRegex,
		IntentExclude: docContextLabels,
	}
	...
	tests := []struct {
		name    string
		command string
		want    bool // true = BLOCK, false = excluded
	}{
		// True positives — must still fire
		{"direct tccutil reset", "tccutil reset Camera", true},
		{"sudo tccutil reset All", "sudo tccutil reset All", true},
		{"compound with direct tccutil", "cd /tmp && tccutil reset Microphone", true},
		// gh --body cases (issue #1778) — must be excluded by DOC_CONTEXT
		{"gh issue create --body inline", `gh issue create --title "FP" --body "block tccutil reset Camera"`, false},
		{"gh issue create -R with --body", `gh issue create -R AI-AgentLens/AI_risk_compliance --label "rule-request,auto-generated" --title "feat(rules): Semgrep rules" --body "Detection: tccutil reset Camera"`, false},
		// compound heredoc (root cause of issue #1778) — must be excluded
		{"cat heredoc at start", "cat > /tmp/body.txt << 'EOF'\ntccutil reset Camera\nEOF", false},
		{"cd && cat heredoc compound", "cd /tmp && cat > /tmp/body.txt << 'EOF'\ntccutil reset Camera\nEOF", false},
		{"cd && tee heredoc compound", "cd /dir && tee /tmp/rule.yaml << 'EOF'\n# tccutil reset Camera\nEOF", false},
		// echo — already covered by DOC_CONTEXT
		{"echo with tccutil reset", "echo 'block tccutil reset Camera'", false},
	}
