...
func matchesEvalRisk(cmd string) bool {
	...
	// python3 -c "..." with double-quoted script: strip inner single-quoted string
	// literals before checking. eval/exec may appear only as Python string data.
	// Must run BEFORE the compound-op split below because python -c arguments
	// often contain semicolons (Python statement separators) that would be split
	// into dangling segments, leaking eval()/exec() patterns from inside strings.
	if pythonInlineDQRe.MatchString(cmd) {
		stripped := innerSingleQuoteRe.ReplaceAllString(cmd, "")
		if idx := strings.Index(stripped, "<<"); idx != -1 {
			stripped = stripped[:idx]
		}
		return evalRiskPattern.MatchString(stripped)
